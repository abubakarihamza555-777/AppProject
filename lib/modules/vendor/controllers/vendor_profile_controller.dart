...
class VendorProfileController extends ChangeNotifier {
  ...
  Future<bool> createOrUpdateVendorProfile({
    ...
    List<int> serviceWards = const [], // Add this parameter
  }) async {
    setLoading(true);
    _clearError();
    ...
    try {
      ...
      final vendorData = {
        ...
        'business_address':
            businessAddress.isNotEmpty ? businessAddress : 'Dar es Salaam',
      ...
      await _supabase.from('users').update(
          {'role': 'vendor', 'is_active': true}).eq('id', currentUser.id);
      ...
      if (e.toString().contains('duplicate key')) {
        print(
            '❌ Duplicate key error - vendor already exists (upsert should handle this)');
  ...
  Future<bool> toggleAvailability() async {
    ...
    try {
      ...
      await _supabase
          .from('vendors')
          .update({'is_active': newStatus}).eq('id', _vendorProfile!.id);
